...
export const getAllProjects = async (): Promise<Project[]> => {
    try {
        ...
        // Ensure all projects have a progress value
        const validatedProjects = projects.map(project => ({
            ...project,
            progress: project.progress ?? 0  // Default to 0 if progress is undefined
        }));
        return validatedProjects.sort((a, b) => a.name.localeCompare(b.name));
    } catch (err) {
...
export const getProjectById = async (id: string): Promise<Project | null> => {
    try {
        ...
        // Ensure the project has a progress value
        return {
            ...project,
            progress: project.progress ?? 0  // Default to 0 if progress is undefined
        };
